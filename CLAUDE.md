...
- **Outbound webhooks**: `notification_config.webhook_enabled`/`webhook_url`/`webhook_secret`; `_send_webhook()` in `notifier.py` POSTs JSON with optional HMAC-SHA256 `X-Hub-Signature-256` header; events: `daily_summary`, `upgrade_complete`, `upgrade_failed`, `upgrade_all_complete`, `security_updates_found`, `reboot_required`; UI in Settings → Notifications
- **Per-trigger webhook toggles**: Notification Triggers table in Settings has a Webhook column alongside Email and Telegram columns; each of the 5 triggers (daily summary, upgrade complete, on error, security updates found, reboot required) has independent `*_email`/`*_telegram`/`*_webhook` boolean columns in `notification_config`; webhook checkboxes are disabled when `webhook_enabled` is false; 11 new DB columns added in this batch (`daily_summary_webhook`, `notify_upgrade_webhook`, `notify_error_webhook`, `notify_security_updates/email/telegram/webhook`, `notify_reboot_required/email/telegram/webhook`)
- **"Security updates found" notification trigger**: `notify_security_updates_found(cfg, db)` in `notifier.py`; fires after every `check-all` when any server has `security_packages > 0`; sends HTML email (red alert), Telegram, and webhook; controlled by `notify_security_updates`/`notify_security_email`/`notify_security_telegram`/`notify_security_webhook` flags
- **"Reboot required" notification trigger**: `notify_reboot_required(cfg, db)` in `notifier.py`; fires after every `check-all` when any server has `reboot_required=True`; sends HTML email (amber alert), Telegram, and webhook; controlled by `notify_reboot_required`/`notify_reboot_email`/`notify_reboot_telegram`/`notify_reboot_webhook` flags
- **`_fetch_server_check_data(db)`** shared helper in `notifier.py` — fetches all servers with their latest `UpdateCheck` and `ServerStats` in one pass; used by `send_daily_summary`, `notify_security_updates_found`, and `notify_reboot_required` to avoid repeated N-query patterns
- **`_send_event_notifications()`** in `scheduler.py` — calls `notify_security_updates_found` and `notify_reboot_required`; invoked from `_job_check_all` between the check and the daily summary
- **Daily summary now includes reboot + EEPROM**: HTML format has a dedicated EEPROM section (amber card) for Pi servers with updates; text and Telegram formats include reboot and EEPROM counts in the summary line and per-server flags; per-channel guards (`daily_summary_email`/`daily_summary_telegram`/`daily_summary_webhook`) are now enforced
...
- **dpkg log history tab**: `GET /api/servers/{id}/dpkg-log` in `backend/routers/dpkg_log.py`; reads `/var/log/dpkg.log`, `dpkg.log.1`, and all `dpkg.log.*.gz` rotations via SSH; parses into structured entries (only `install`/`upgrade`/`remove`/`purge` actions — skips `status`/`configure`/`trigproc`); query params: `package` (substring), `action`, `days`, `limit`, `offset`; results sorted newest-first; fetched on demand (load button) to avoid adding SSH overhead to every page load; new "dpkg Log" tab in `ServerDetail.tsx` with action colour badges (green=install, cyan=upgrade, amber=remove, red=purge) and filters for package name, action type, and time window
- **`.deb` package installation**: two paths — URL download and direct file upload; `POST /api/servers/{id}/validate-deb-url` does a HEAD request from the dashboard container to check the URL is reachable and returns a `.deb`; `POST /api/servers/{id}/upload-deb` accepts multipart `.deb` upload and SFTP's it to `/tmp/` on the remote via `asyncssh.start_sftp_client()`; `WS /api/ws/install-deb/{server_id}` streams `wget` (URL) or `dpkg -i` (pre-uploaded) + `apt-get install -f` output; remote path restricted to `^/tmp/[a-zA-Z0-9._\-+]+\.deb$` to prevent path injection; triggers background check after successful install; "+ Install .deb" button in Packages tab header opens `DebInstallModal` (portal-based, two-tab: URL / Upload)
- **Apt repo management**: new "Apt Repos" tab in `ServerDetail.tsx`; `backend/routers/apt_repos.py` — `GET /api/servers/{id}/apt-repos` reads all source files in one SSH call using a `===FILE==>` marker protocol (`_READ_CMD` + `_parse_files()`); `PUT /api/servers/{id}/apt-repos` writes content via `sudo tee` with stdin to avoid shell escaping; `DELETE /api/servers/{id}/apt-repos` removes files from `sources.list.d` only; `WS /api/ws/apt-repos-test/{id}` streams `sudo apt-get update` output; strict path allowlist (`_allowed_path()`, `_deletable_path()`) blocks path traversal — only `/etc/apt/sources.list` and `/etc/apt/sources.list.d/[safe].{list,sources}` accepted; frontend `AptReposTab` shows per-file tabs with amber `•` dirty indicator, monospace textarea editor, Save/Delete buttons, inline "New File" form (validated filename regex), and "Test with apt-get update" terminal; unsaved edits tracked per path in `pendingEdits` Record so switching tabs doesn't lose work
